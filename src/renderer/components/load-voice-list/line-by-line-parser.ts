...
import {VoiceItem, VoiceListParser} from '../../types';

class LineByLineParser implements VoiceListParser {
    public async parse(content: string): Promise<VoiceItem[]> {
        ...
        return content
            ...
            .filter(([displayText, fileSystemName]): boolean => {
                ...
                if (displayTextSet.has(displayText)) {
                    throw new Error(`Duplicate voice items to record: ${displayText}`);
                }
            ...
            .map(([displayText, dedupedFileSystemName]): VoiceItem => ({
                displayText,
