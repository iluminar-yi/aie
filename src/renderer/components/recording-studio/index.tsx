...
const VoiceItemSelector = ({
  ...
  return (
    <Fragment>
      {recordingList.map(
        (voiceItem, i): ReactElement => {
          return (
            <div
              key={i}
              onClick={(): void => setVoiceItemIndex(i)}
              ref={i === voiceItemIndex ? currentVoiceItemRef : undefined}
              style={!voiceFileExistList[i] ? { fontStyle: 'italic', fontWeight: 'bolder' } : undefined}>
...
const RecordingStudio = ({
  ...
  onRecorderStopRef.current = (): void => {
    ...
    const { fileSystemName } = recordingFileState[scales[scaleIndex]][voiceItemIndex];
  ...
  useEffect((): (() => void) => {
    ...
    updateFileStatusRef.current().then(([changed]) => {
      if (
        ...
        recordingFileState[scales[scaleIndex]][voiceItemIndex].audioExists
      ) {
        return (async (): Promise<void> => {
          const filePath = path.join(
            projectFolder,
            scales[scaleIndex],
            `${recordingFileState[scales[scaleIndex]][voiceItemIndex].fileSystemName}.wav`,
          );
  ...
  }, [audioPlayBackRef, projectFolder, recordingFileState, scaleIndex, scales, studioState, voiceItemIndex]);
          ...
          <Grid columns={16}>
            <Grid.Row divided={true}>
              ...
              <Grid.Column width={11}>
                ...
                <Grid.Row>
                  <Button
                    onClick={(): void => {
                      setVoiceItemIndex((voiceItemIndex - 1 + recordingList.length) % recordingList.length);
                    }}
                    disabled={studioState !== StudioState.IDLE}>
                    Previous Voice
                  </Button>
                  <Button
                    onClick={(): void => {
                      setVoiceItemIndex((voiceItemIndex + 1) % recordingList.length);
                    }}
                    disabled={studioState !== StudioState.IDLE}>
                    Next Voice
                  </Button>
            ...
            <Grid.Row>
              {studioState === StudioState.RECORDING ? (
                ...
                <pre>
                  {recordingFileState[scales[scaleIndex]] &&
                    JSON.stringify(recordingFileState[scales[scaleIndex]][voiceItemIndex], null, 2)}
                </pre>
              )}
